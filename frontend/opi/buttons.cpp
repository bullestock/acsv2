...
{
    logger.log_verbose(fmt::format("set pin {} to input", pin));
    int fd = open("/sys/class/gpio/export", O_WRONLY);
    if (fd == -1)
        logger.fatal_error("Unable to open /sys/class/gpio/export");

    const auto name = fmt::format("{}", pin);
    if (write(fd, name.c_str(), name.size()) != name.size())
        logger.fatal_error("Error writing to /sys/class/gpio/export");
    ...
    if (write(fd, "in", 2) != 2)
        logger.fatal_error(fmt::format("Error writing to {}", dir));
    close(fd);
}
...
bool Buttons::read_pin(int pin, bool do_log)
{
    ...
    if (fd == -1)
    {
        if (!do_log)
            return false;
        logger.fatal_error(fmt::format("Unable to open {}: {}", name, errno));
    }
    char c;
