...
docker build -t bullestock/acsv3:$serial -f docker/Dockerfile --build-arg GIT_COMMIT=`git rev-parse HEAD` --build-arg BUILD_DATE="`date '+%Y%m%d %H:%M'`" --build-arg VERSION_NUMBER=$serial .
