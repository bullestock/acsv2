        ...
        try:
            ...
            for m in members:
                ...
                try:
                    ...
                    logger.info(f'SYNCH: Member {name} (ID {number}, username {login}) does not exist')
                    u = None
                if u:
                    #logger.info(f'SYNCH: Member {name} (ID {number}) already exists')
                    ...
                    if login != fl_login:
                        try:
                            ...
                            logger.info(f'SYNCH: Deleting old member {fu.first_name} {fu.last_name}')
                            fu.delete()
                        except Member.DoesNotExist:
                            pass
                    ...
                    u.set_password(None)
                if not u.is_active:
                    logger.info(f'SYNCH: Member {number} ({first_name} {last_name}) is now active')
                u.is_active = True
                u.fl_id = number
                u.fl_int_id = id
            ...
            Path('/opt/app/acsv3/monitoring/acs-sync-status').touch()
        except Exception as e:
            logger.info("SYNCH: Exception: {0} {1}".format(e, traceback.format_exc()))
            pass
